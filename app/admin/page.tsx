...
export default function AdminDashboard() {
    ...
    const getDisplayedProjects = (projects: typeof completedProjects, showAll: boolean) => {
        return showAll ? projects : projects.slice(0, 5);
    };
            ...
            <div className="grid gap-6 md:grid-cols-3 items-start">
                ...
                <Card className="hover:shadow-lg transition-shadow duration-300 flex flex-col">
                    <CardHeader className="pb-3">
                        <CardTitle className="text-lg font-medium text-green-600">Completed Projects</CardTitle>
                    </CardHeader>
                    <CardContent className="flex-1">
                        <ul className="space-y-3">
                            {getDisplayedProjects(completedProjects, showAllCompleted).map((project) => (
                                <li key={project.id}>
                                    <Link href={`/admin/projects/${project.id}`} className="block group p-3 rounded-lg hover:bg-accent hover:shadow-sm transition-all border border-transparent hover:border-border/50">
                ...
                <Card className="hover:shadow-lg transition-shadow duration-300 flex flex-col">
                    <CardHeader className="pb-3">
                        <CardTitle className="text-lg font-medium text-blue-600">Running Projects</CardTitle>
                    </CardHeader>
                    <CardContent className="flex-1">
                        <ul className="space-y-3">
                            {getDisplayedProjects(runningProjects, showAllRunning).map((project) => (
                                <li key={project.id}>
                                    <Link href={`/admin/projects/${project.id}`} className="block group p-3 rounded-lg hover:bg-accent hover:shadow-sm transition-all border border-transparent hover:border-border/50">
                                        <div className="flex items-start gap-3">
                                            <div className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-blue-500 animate-pulse" />
                                            <div>
                                                <h3 className="font-semibold text-foreground group-hover:text-primary transition-colors">{project.title}</h3>
                                                <p className="text-xs text-muted-foreground line-clamp-2">{project.description}</p>
                                            </div>
                ...
                <Card className="hover:shadow-lg transition-shadow duration-300 flex flex-col">
                    <CardHeader className="pb-3">
                        <CardTitle className="text-lg font-medium text-orange-600">Upcoming Projects</CardTitle>
                    </CardHeader>
                    <CardContent className="flex-1">
                        <ul className="space-y-3">
